...
public class HitController {
   ...
   @Operation(summary = "좋아요 토글", 
               description = "게시글의 좋아요를 추가하거나 제거합니다. 이미 좋아요가 있으면 제거하고, 없으면 추가합니다.")
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "토글 성공", 
                    content = @Content(schema = @Schema(
                        description = "좋아요 상태 및 개수 정보",
                        example = "{\"action\": \"added\", \"isHit\": true, \"hitCount\": 5}"))),
   ...
   @Operation(summary = "좋아요 수 조회", 
               description = "특정 게시글의 전체 좋아요 수를 조회합니다.")
    @ApiResponses({
   ...
   @Operation(summary = "좋아요 상태 확인", 
               description = "특정 사용자가 특정 게시글에 좋아요를 했는지 확인합니다.")
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "조회 성공", 
                    content = @Content(schema = @Schema(
                        type = "boolean",
