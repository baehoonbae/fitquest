<template>
  <div class="max-w-[900px] mx-auto px-2 flex flex-col gap-2" @keyup.esc="showRandomUsers = false">
    <!-- 랜덤 유저 버튼 -->
    <button @click="showRandomUsers = true"
      class="flex items-center justify-center gap-2 py-3 px-4 bg-black hover:bg-gray-700 text-white rounded-[15px] transition-colors">
      <span class="material-icons">group_add</span>
      <span>운동 메이트를 만나보세요</span>
    </button>
    <!-- 메인 컨텐츠 영역 -->
    ...
    <Transition name="modal">
      <div v-if="showRandomUsers" class="fixed inset-0 z-[100]">
        <RandomUserList @closeModal="showRandomUsers = false" />
      </div>
    </Transition>
  </div>
</template>
