    ...
    <div class="h-screen flex flex-col" @keydown.enter="regist">
        <div class="w-[950px] max-w-full px-4 mx-auto mt-0">
            ...
            <form @submit.prevent="regist" class="space-y-2">
                <div>
                    <input type="text" placeholder="닉네임 등록" :disabled="isLoading"
                        class="w-full px-4 py-3 font-semibold rounded-lg focus:outline-none bg-[#F2F2F2]"
                        v-model="user.name" @blur="checkName" />
                    <p v-if="isNameEmpty && isNameBlurred" class="text-sm text-red-400 pl-2 pt-1">닉네임을 입력해주세요.</p>
                    <p v-else-if="isNameDuplicated && isNameBlurred" class="text-sm text-red-400 pl-2 pt-1">사용할 수 없는 닉네임입니다.</p>
                </div>
                <div>
                    <input type="email" placeholder="이메일 등록" :disabled="isLoading"
                        class="w-full px-4 py-3 font-semibold rounded-lg focus:outline-none bg-[#F2F2F2]"
                        v-model="user.email" @blur="checkEmail" />
                    <p v-if="isEmailEmpty && isEmailBlurred" class="text-sm text-red-400 pl-2 pt-1">이메일을 입력해주세요.</p>
                    <p v-else-if="isEmailDuplicated && isEmailBlurred" class="text-sm text-red-400 pl-2 pt-1">사용할 수 없는
                        이메일입니다.</p>
                </div>
                <div class="relative">
                    <input :type="showPassword ? 'text' : 'password'" placeholder="비밀번호 등록" :disabled="isLoading"
                        class="w-full px-4 py-3 font-semibold rounded-lg focus:outline-none bg-[#F2F2F2]"
                        v-model="user.password" @blur="checkPassword" />
                    <button type="button" @click="showPassword = !showPassword"
                        class="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-500">
                        <EyeIcon v-if="showPassword" class="w-5 h-5" />
                        <EyeSlashIcon v-else class="w-5 h-5" />
                    </button>
                    <p v-if="isPasswordEmpty && isPasswordBlurred" class="text-sm text-red-400 pl-2 pt-1">패스워드를
                        입력해주세요.</p>
                </div>
            </form>
...
const isFormValid = computed(() => {
    return !isNameEmpty.value && !isEmailEmpty.value && !isPasswordEmpty.value && !isNameDuplicated.value && !isEmailDuplicated.value;
});
