  ...
  <div v-if="isOpen">
    ...
    >
      ...
      <div class="bg-white rounded-lg shadow-xl w-11/12 max-w-lg p-6" @click.stop>
        ...
        <!-- 게시물 목록 -->
        <div class="mt-4">
          <ul v-if="recentNews.length > 0" class="divide-y divide-gray-200">
            <li
              v-for="news in recentNews"
              :key="news.id"
              class="py-3 hover:bg-gray-50 transition-colors duration-150"
              @click="navigateToNews(news.link)"
            >
              <button class="flex flex-col w-full">
                <span class="text-gray-800 font-medium">{{ news.title }}</span>
                <span class="text-sm text-gray-500 mt-1">{{ news.postdate }}</span>
              </button>
            </li>
          </ul>
